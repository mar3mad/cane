%% Preamble
clc, clear;
% See section Extensions - Large deflections
%
...
propStr.b = .1; % width
propStr.h = .1; % height
propStr.A = propStr.b*propStr.h; % square cross-section
propStr.I = propStr.A*(propStr.h^2)/12; % moment of inertia
...
w_analytical(X) = q*X^2/(24*E*I)*(6*L^2 - 4*L*X + X^2);
w_analytical(X) = subs(w_analytical(X), {q, E, I, L}, ...
    {propStr.q0, propStr.E, propStr.I, propGeom.XL - propGeom.X0});
...
% Dirichlet boundary conditions (cantilever beam fixed on its left end)
XBC = [repmat(XComp(1, 1), 1, 2) repmat(XComp(1, 2), 1, 2)]; % repmat(XComp(1, 2), 1, 2)
WBC = ones(1, numBC)*w0;
...
function [gradients,loss] = modelGradients ...
    ...
    % Compute lossF. Enforce the Euler-Bernoulli equation
    f = propStr.E*propStr.I*Wxxxx - 3/2*propStr.E*propStr.A*Wx.^2.*Wxx - propStr.q0;
    zeroTarget = zeros(size(f), 'like', f);
    lossF = mse(f, zeroTarget);
    
    % Compute the system response at the beam's ends
    dlU0Pred = model(parameters, dlXBC);
    lossU = mse(dlU0Pred(1, 1), dlWBC(1, 1));

    % Compute the first derivative of the vertical deflection (rotation) 
    % at the left edge of the beam (simple support)
    dlWBCx = dlgradient(sum(dlU0Pred,'all'), dlXBC, 'EnableHigherDerivatives', true);
    lossDwDX = mse(dlWBCx(1, 2), dlWBC(1, 2));

    % Compute the second and the third derivatives of the vertical
    % deflection at the right edge of the beam (free end)
    dlWBCxx = dlgradient(sum(dlWBCx,'all'), dlXBC, 'EnableHigherDerivatives', true);
    dlWBCxxx = dlgradient(sum(dlWBCxx,'all'), dlXBC, 'EnableHigherDerivatives', true);
    lossDDwDDX = mse(dlWBCxx(1, 3), dlWBC(1, 3));
    lossDDDwDDDX = mse(dlWBCxxx(1, 3), dlWBC(1, 4));
    
    % Combine losses into the general loss function
    loss = propLearning.rhoR*(1/propLearning.numR)*lossF + ...
        propLearning.rhoB*(1/propLearning.numB)*(lossU + lossDwDX + lossDDwDDX + lossDDDwDDDX); % lossDDwDDX, lossDDDwDDDX
